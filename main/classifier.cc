...
float g_dense_weights[NUM_CLASSES][NUM_INPUTS];
float g_dense_bias[NUM_CLASSES];
...
void update_classifier_weights(const float* values, int value_count) {
    int expected = NUM_CLASSES * NUM_INPUTS ;
    ...
    for (int i = 0; i < NUM_CLASSES; ++i) {
        for (int j = 0; j < NUM_INPUTS; ++j) {
            g_dense_weights[i][j] = values[i * NUM_INPUTS + j];
        }
    }
}
