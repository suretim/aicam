...
// TFLite Micro headers (assume properly added in component)
...
void tflite_init() {
    ...
    const tflite::Model* model = tflite::GetModel(g_student_encoder_tflite); // from model_data.h
    ...
    printf("TFLite initialized. Input bytes=%d, output bytes=%d\n", input_tensor->bytes, output_tensor->bytes);
}

// Run encoder on preprocessed image buffer
// image_data: float32 normalized data matching encoder expectation, length = input_tensor->bytes/4
// feature_out: float array of length FEATURE_DIM
